...
## Springboot操作Redis 工具类封装

## 存储JSON格式数据

> 此时我们就会想到有没有什么办法让Redis中存储的数据变成标准的JSON格式,不然存储格式为java序列化结果。在Spring Cache可以设置一定的过期时间，不设置过期时间容易产生很多不必要的缓存数据。
...
publicclass RedisConfig extends CachingConfigurerSupport {
     ...
     * redis数据库自定义key
     */
    publicstaticfinal String REDIS_KEY_DATABASE="mall";
    ...
    public RedisCacheManager redisCacheManager(RedisConnectionFactory redisConnectionFactory) {
        ...
        returnnew RedisCacheManager(redisCacheWriter, redisCacheConfiguration);
    }
}
```
...
publicclass MallTinyApplication {
 ...
 */
@Service
publicclass PmsBrandServiceImpl implements PmsBrandService {
    @Autowired
...
https://github.com/andanyoung/springboot/tree/master/swagger3/redis