package andanyoung.springboot.swagger3.web.controller;

import andanyoung.springboot.swagger3.web.model.User;
import andanyoung.springboot.swagger3.web.model.UserVO;
...
public class UserController {
  ...
  public User getUser(
    ...
    return list.stream()
        .filter(u -> u.getId().equals(userId) && u.getAddress().equals(address))
  ...
  public User getUser2(@RequestBody UserVO userVO) {
    ...
    return list.stream()
        .filter(u -> u.getId().equals(userVO.getId()) && u.getAddress().equals(userVO.getAddress()))
        .filter(Objects::nonNull)
        .findFirst()
        .orElse(new User());
  }
  ...
  @ApiImplicitParams({@ApiImplicitParam(name = "name", value = "上传人")})
